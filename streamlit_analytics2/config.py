...
# Default configuration (secrets loaded from env)
DEFAULT_CONFIG = {
    ...
    "logs": {"verbose": False},
    "access": {"unsafe_password": os.getenv("UNSAFE_PASSWORD", "")},
    "firestore": {
        "enabled": False,
        "firestore_key_file": os.getenv("FIRESTORE_KEY_FILE", ""),
        "firestore_project_name": "",
        "firestore_collection_name": "streamlit_analytics2",
        # "firestore_document_name": "data",
        "streamlit_secrets_firestore_key": os.getenv("STREAMLIT_SECRETS_FIRESTORE_KEY", ""),
    },
    "session": {"session_id": ""},
}
...
def show_config():
    ...
    password = st.text_input(
        "Access Password",
        value=os.getenv("UNSAFE_PASSWORD", config["access"].get("unsafe_password", "")),
        type="password",
    )
    st.divider()
    ...
    firestore_key_file = st.text_input(
        ...
        value=os.getenv("FIRESTORE_KEY_FILE", config["firestore"].get("firestore_key_file", "")),
    )
    ...
    firestore_secret_key = st.text_input(
        ...
        value=os.getenv("STREAMLIT_SECRETS_FIRESTORE_KEY", config["firestore"].get("streamlit_secrets_firestore_key", "")),
        type="password",
    )
    st.divider()
