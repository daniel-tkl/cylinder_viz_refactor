...
def show_results(data, reset_callback, unsafe_password=None):  # noqa: F811
    ...
    st.markdown(
        """
        You found a secret section analytics.
    ...
    if show:
        ...
        for i in data["widgets"].keys():
            ...
            if type(data["widgets"][i]) is dict:
                # Normalize mixed keys to strings so Arrow conversion stays stable.
                selected_values = [str(value) for value in data["widgets"][i].keys()]
                st.dataframe(
                    pd.DataFrame(
                        {
                            "widget_name": i,
